...
func (z *ZipFile) AddAll(dir string, includeCurrentFolder bool, except []string) error {
	dir = path.Clean(dir)
	return addAll(dir, dir, includeCurrentFolder, except, func(info os.FileInfo, file io.Reader, entryName string) (err error) {
...
func (t *TarFile) AddAll(dir string, includeCurrentFolder bool, except []string) error {
	dir = path.Clean(dir)
	return addAll(dir, dir, includeCurrentFolder, except, func(info os.FileInfo, file io.Reader, entryName string) (err error) {
...
func addAll(dir string, rootDir string, includeCurrentFolder bool, except []string, writerFunc ArchiveWriteFunc) error {
	...
	for _, info := range fileInfos {
		...
		if info.IsDir() {
			addAll(full, rootDir, includeCurrentFolder, except, writerFunc)
		}
	}
